...
multi_1D_plot <- function (i_site,i_mod=1:9, set_lim, multipl_fact=1,pal_name='Set1'){
  ...
  df_summary$rfrz_2sig_pct <- df_summary$rfrz_2sig/df_summary$rfrz_avg*100
  ...
  df_summary$runoff_2sig_pct <- df_summary$runoff_2sig/df_summary$runoff_avg*100
  print(df_summary)
  ...
  plot_func <- function(df,y_lab_text){
    ...
    h <- ggplot(data = df2, 
              ...
              geom_point(data = df, 
                    aes(x=factor(year), y = value, colour=variable), position=position_jitterdodge(),  size=2,alpha=0.6)+
...
g <- ggplot(data = perc_df_m, aes(x = time_select, y = value, color  = variable)) +
  ...
  grids()+
      ...
      ggsave(filename=paste0("Plots/percolation.jpg"),
             plot=g,device = "jpeg",
                      width = 20, height = 8, dpi = 300, units = "cm")
