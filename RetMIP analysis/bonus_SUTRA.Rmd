...
multiplot <- function (models,var_name,clab_text,i_site,set_lim,offset,multipl_fact){
  ...
  pList <- lapply(1:length(models), function(i_mod){
    p <- ggplot(data = df,
      ...
      ggtitle(paste0(letters[i_mod],") ",sites[i_mod]))
  ...
  p_final <- grid.arrange(grobs=pList, nrow=1,
    ...
    bottom="Year",
    right=" ",
    vp=viewport(x=0, just="left", width=1, height=1),
    widths = c(1.2,1.2))
  print("Saving...")
...
```{r rho 1}
  multiplot(models,var_name="lwc",clab_text='lwc (kg m-3)',i_site = 1,set_lim = c(0,5000))
```
