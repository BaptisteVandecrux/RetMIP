...
# Plotting
  ...
  rfrz_mod <- rfrz_mod*multipl_fact
    ...
    if (strcmp(models[[i_mod]],"IMAUFDM")) {
  rfrz_mod <- pmax(rfrz_mod,0)}
...
```{r Subsurface temperature}
  i_site <- 1
  if(i_site==1) { name_file<-'KANU_PROMICE' } else {name_file <- sites[i_site]}
  ...
  time<-1:length(time)
  TT <- repmat(time,size(T_firn)[1],1)
  therm_num <- repmat(matrix(1:size(T_firn)[1],nrow=size(T_firn)[1]),1,length(time))

  dat <- t(rbind(as.vector(TT[,seq(0, length(time), by = 100)]),
         as.vector(depth[,seq(0, length(time), by = 100)]),
         as.vector(T_firn[,seq(0, length(time), by = 100)]),
         as.vector(therm_num[,seq(0, length(time), by = 100)])))
...
p1 <- ggplot() + geom_point(aes(y = z, x = x, colour = therm_num), size=1.5,
                           data = dat[dat$therm_num==1:8,])
p1


fld <- interp(dat$x, dat$y, dat$z)
...
p <- ggplot(gdat) + 
  ...
  geom_point(data = dat, aes(y = y, x = x, colour = therm_num), size=0.5)+
